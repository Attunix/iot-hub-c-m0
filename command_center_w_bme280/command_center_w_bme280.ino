...
*/
static const char ssid[] = "AttunixGuest";
static const char pass[] = "a77un!><";
static const char* connectionString = "HostName=RaspPi47914.azure-devices.net;DeviceId=raspy;SharedAccessKey=R3YaruxyEQ0eCi/JXglY1A==";
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("Azure Command Center Example Sketch.");
...
void loop() {
  ...
  if (IS_TASK_TIME(Curr_time_ms__u32, Sensor_read_next_ms__u32)) {
    ...
    rem_ctrl_http_send_data(Temp_c__f, Humi_pct__f);
...
void initTime() {
    ...
    while (true) {
        ...
        if (epochTime == (time_t)-1) {
            Serial.println("Fetching NTP epoch time failed! Waiting a few seconds before retry.");
            delay(3000);
        } else {
